...
def test_package_import_does_not_preload_cli() -> None:
    ...
    subprocess.run(
        [
            sys.executable,
            "-c",
            "import sys, venus_observability; "
            "assert 'venus_observability.__main__' not in sys.modules",
        ],
        check=True,
    )
...
def test_signal_shutdown_runs_cleanup_once() -> None:
    ...
    with (
        ...
        patch(
            "signal.signal", side_effect=lambda signum, handler: handlers.update({signum: handler})
        ),
        patch("time.sleep", side_effect=lambda _: handlers[signal.SIGTERM](signal.SIGTERM, None)),
